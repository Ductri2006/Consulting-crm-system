...
token hashes, and staff contact details are not exposed. Step 30 reuses the same
models for Portal Updates: safe case-history action/status fields,
appointments, customer-visible documents, current portal-account download
audits, and account readiness are normalized into a feed without exposing raw
internal activity rows.
...
Step 30 exposes them only through the Admin/Manager Activity Center, scoped by
the authenticated user's `organizationId` and combined with other safe activity
sources for timeline review.
...
- Activity Center filtering by actor, action, entity, search text, and creation
  time, always under the authenticated workspace
...
- Portal updates must scope by portal-account `organizationId` and `customerId`,
  validate optional `caseId` ownership, exclude raw internal `ActivityLog`
  descriptions, and omit internal notes, storage metadata, token hashes,
  password hashes, IP addresses, and user-agent data.
